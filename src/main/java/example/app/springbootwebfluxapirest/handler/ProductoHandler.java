package example.app.springbootwebfluxapirest.handler;
    ...
    public Mono<ServerResponse> upload(ServerRequest request) {
        ...
        return request.multipartData().map(multipart -> multipart.toSingleValueMap().get("file"))
                ...
                .flatMap(filePart -> productoService.findById(id)
                        .flatMap(p -> {
                    p.setFoto(UUID.randomUUID().toString() + "-" +filePart.name()
                            .replace(" ", "")
                            .replace(":", "")
                            .replace("\\", ""));
                    return filePart.transferTo(new File(path + p.getFoto())).then(productoService.save(p));
